...
従来の方法より変更箇所を発見しやすく、変更箇所の個数も削減できる。
変更には、生成規則の追加、削除がある
...
どのメソッドまたはデリゲートを適用させているのかがC\#版Parsecよりシーケンスを把握しやすい。
...
\begin{table}[H]
 \centering
 \caption{用語定義}　\label{tb:result}
 \begin{tabular}{|p|p|p|p|} \hline
            & 1回目  & 2回目  & 3回目\\ \hline \hline
  自作パーサ    & 2分32秒&　2分24秒&　2分25秒 \\ \hline
　　C\#版Parsec& 2分40秒&　2分40秒&　2分40秒  \\ \hline
　　従来手法     & 2分30秒&　2分29秒&　2分26秒
  \hline
 \end{tabular}
\end{table}

順位は自作パーサ、従来手法、C\#版Parsecの順であった。
自作パーサは、基本的に文字列をそのまま扱うのではなく文字に分解して処理しているため、
文字列を対象とする正規表現を使用する従来手法より速くなったと考えられる。
...
\subsubsection{解析能力}
生成規則の表現力は、どの方法も違いが見られない。しかしながら、従来の方法で使用されていた正規表現では、
括弧のネストを正確にパースする
\footnote
{
.NET Frameworkの正規表現は、通常のものより柔軟かつ強力にできており、
右から左への一致処理や再帰を利用したマッチングが可能\cite{msdn}。
}
のに、現状以上に複雑な正規表現となってしまう
\footnote{従来の方法では、括弧のネストをパースできない。また、括弧のネストに}。
