...
これらOSのトレースログの可視化のツールは，特定の環境(OSやデバッグハード
ウェア)を対象としているため，可視化対象となるトレースログの形式が固定さ
れている．
%
そのため，FMPカーネルの様な可視化ツールが用意されていない環境のトレー
スログの可視化に，これらの既存ツールを用いることができず，汎用性に乏し
いという問題がある．
%
...
ルとその問題点について述べる．また，標準化されたトレースログの形式とツー
ルのOSのトレースログの可視化への流用について述べる．
...
Paj\`{e} 形式では，VCD形式と比較すると表現能力は高いが，状態の定義と状態
毎の色指定程度の自由度しかないため，同様に複雑な可視化表現は不可能である．
PthreadプログラムのトレースログをVisual Trace Explorerで可視化表示した例
を図\ref{fig:VITE}に示す．

\begin{figure}[tb]
\begin{center}
\scalebox{0.35}{\includegraphics{img/VITE.eps}}
\caption{Visual Trace ExplorerによるPthreadプログラムのトレースログ可視化}
\label{fig:VITE}
\end{center}
\end{figure}
...
{\tt Task}であり，タスクIDが1のタスクはTASK1という名前のリソースとする
...
のうち，属性のタスク状態({\tt state})が実行状態であるリソースを指定して
いる．
...
リソースヘッダファイルには，可視化対象の環境(OS等)のリソースタイプの定義
を記述する．リソースタイプの定義には，リソースタイプの名前，表示名，属性，
振る舞いを記述する．
...
属性の初期値を指定している．\verb|TASK1|は，タスクIDが1であり，初期状態
が休止状態(DORMANT)である．
...
なお，図\ref{fig:log_ASP}と異なり，タスクの指定を，``TASK1''という文字列
使わず，リソースタイプTaskのタスクIDの属性の条件でTASK1を指定している
(Task(id==1))．直接文字列を生成したい場合は，変換ルールで\$RES\_NAMEの置
換マクロを用いればよい．
%
具体的には，図\ref{standartFormatTraceLogSample}の4行目や7行目を
Task(id==\$\{id\})を\$RES\_NAME\{\}で囲む．
...
\begin{figure}[ht]
\begin{center}
\scalebox{0.5}{\includegraphics{img/aspTLVscreenShot_pri.eps}}
\caption{タスクの優先度でグループ化されたツリービュー形式の表示}
...
また，既存ツールと比較した，TLVの制約や課題について述べる．
