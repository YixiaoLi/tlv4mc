...
\begin{itembox}[l]{文字列可視化のログ書式}
  \paragraph{syslog}
  syslog(``applog id \%s \%s.'',\textit{id},\textit{str});
  \paragraph{API}
  void applog\_str(const char *\textit{id}, const char *\textit{str})
  \paragraph{ログ出力}
  \verb|[| \textit{time} \verb|]| applog id \textit{id} \textit{str}. 
  \begin{description}
    \item[\textit{time}] 時刻 \verb|[0-9a-zA-Z]+|
    \item[\textit{id}] 表示行ID \verb|[^\. ]+|
    \item[\textit{str}] 出力文字列 \verb|[^\.]*|
  \end{description}
\end{itembox}
\begin{itembox}[l]{タスク文字列可視化のログ書式}
  \paragraph{syslog}
  syslog(``applog task \%s \%s.'',\textit{tid},\textit{str});
  \paragraph{API}
  void applog\_tsk(const char *\textit{tid}, const char *\textit{str})
  \paragraph{ログ出力}
...
以下の記述は、文字列可視化を行う際に、ユーザがリソースファイルに追加するものである。
