...
グプロファイラ\cite{LTTV,Chime}などが存在する．


これら既存のツールは，特定の環境(OSやデバッグハードウェア)を対象としてい
...
いという問題がある．
%
また，可視化表示する項目やその表現方法が固定である場合が多く，拡張性に
乏しいという問題もある．例えば，OSをサポートしている可視化ツールは，タ
スクやセマフォといったOSのオブジェクトの状態を可視化すること可能である．
%
OS上で動作するアプリケーションの開発時には，これらの情報に加えて，アプリ
ケーションの状態（モード・進捗状況）や重要な変数の値といったアプリケーシ
ョン固有の情報を可視化したい場合がある．
%
...
\begin{figure}[tb]
\begin{center}
\scalebox{0.6}{\includegraphics{img/aspTLVscreenShot_pri.eps}}
...
また，既存ツールと比較した場合の，TLVの制約や課題について述べる．
...
み込まれた可視化ツール（以下，統合可視化ツール）と比較して，ユーザー操作
の煩雑性，処理速度，機能的な制約がある．

ユーザー操作の煩雑性に関しては，統合可視化ツールの場合は，ツール内でメニ
ュー等で可視化を選択すると，自動的に可視化が可能である．一方，TLVの場合
は，一旦トレースログをを保存してから，TLVを起動して，読み込ませる必要が
ある．また，トレースログと共にアプリケーション毎にリソースファイルが必要
となるが，リソースファイルを自動的に生成するしくみを用意できない場合は，
ユーザーが手作業で用意する必要がある．

処理速度に関しては，統合可視化ツールでは，ログの形式や可視化項目を固定化
しているのに対して，TLVでは，変換ルールによる標準形式変換や可視化ルール
による図形データ生成が必要であるため，TLVの方が可視化までの時間は長くな
ってしまう．

機能的な制約としては，統合可視化ツールには，プロセッサやシミュレータを停
止してその時点までのトレースログを可視化した後，処理を再開して得られたト
レースログを追加で可視化することが可能なものがある．一方，現状のTLVでは，
トレースログを可視化表示している状態で，続きのトレースログを新たに読み込
ませて可視化表示することはできない．
