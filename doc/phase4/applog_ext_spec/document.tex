...
\begin{description}
  ...
  \item[端点付きユーザ定義状態可視化] 上記の可視化に加え、開始と終了があるもの。関数コールを想定
\end{description}
...
\begin{itembox}[l]{文字列可視化のログ書式}
  \paragraph{syslog}
  syslog("applog str : ID \%s : \%s.",\textit{rid},\textit{str});
  \paragraph{API}
  void applog\_str(const char *\textit{rid}, const char *\textit{str})
  \paragraph{ログ出力}
  \verb|[| \textit{time} \verb|]| applog str : ID \textit{rid} : \textit{str}. 
  \begin{description}
...
applog_str("1", "foo");
\end{verbatim}
このようなログ出力および次のようなリソースファイル定義を行った場合、\figref{fig:str}のように可視化を行う。
\begin{itembox}[l]{リソースファイル}
\begin{verbatim}
"APP_LOG":{
    "Type":"ApplogStr",
    "Attributes":{
...
\begin{verbatim}
syslog("applog strtask : TASK 1 : FOO.");
\end{verbatim}
または
\begin{verbatim}
applog_strtsk("1", "FOO");
