 ...
 *  並行実行されるタスクへのメッセージ領域
 */
volatile char_t	message[TNUM_PRCID][3];
...
trace_log_test(void)
{
	...
	/* レディキューの回転 */
	SVC_PERROR(mrot_rdq(MID_PRIORITY, 1));
	SVC_PERROR(mrot_rdq(MID_PRIORITY, 2));
	dly_tsk(10);
	SVC_PERROR(mrot_rdq(MID_PRIORITY, 1));
	SVC_PERROR(mrot_rdq(MID_PRIORITY, 2));
	dly_tsk(10);
	...
	/* タスク例外起動と終了 */
	SVC_PERROR(ras_tex(TASK1_1, 0x0001U));
	SVC_PERROR(ras_tex(TASK2_1, 0x0001U));
	dly_tsk(10);
	server_req = 0x11 << 16 | SERVER_REQ_TER_TSK;
	SVC_PERROR(snd_dtq(SERVER_DTQ1, server_req));
	server_req = 0x21 << 16 | SERVER_REQ_TER_TSK;
	SVC_PERROR(snd_dtq(SERVER_DTQ2, server_req));
	dly_tsk(10);

	SVC_PERROR(ras_tex(TASK1_2, 0x0001U));
	SVC_PERROR(ras_tex(TASK2_2, 0x0001U));
	dly_tsk(10);
	server_req = 0x12 << 16 | SERVER_REQ_TER_TSK;
	SVC_PERROR(snd_dtq(SERVER_DTQ1, server_req));
	server_req = 0x22 << 16 | SERVER_REQ_TER_TSK;
	SVC_PERROR(snd_dtq(SERVER_DTQ2, server_req));
	dly_tsk(10);

	SVC_PERROR(ras_tex(TASK1_3, 0x0001U));
	SVC_PERROR(ras_tex(TASK2_3, 0x0001U));
	dly_tsk(10);
	server_req = 0x13 << 16 | SERVER_REQ_TER_TSK;
	SVC_PERROR(snd_dtq(SERVER_DTQ1, server_req));
	server_req = 0x23 << 16 | SERVER_REQ_TER_TSK;
	SVC_PERROR(snd_dtq(SERVER_DTQ2, server_req));
	dly_tsk(20);
	...
	/* 全タスク起床 */
	SVC_PERROR(wup_tsk(TASK1_1));
	SVC_PERROR(wup_tsk(TASK2_1));
	dly_tsk(10);
	SVC_PERROR(wup_tsk(TASK1_2));
	SVC_PERROR(wup_tsk(TASK2_2));
	dly_tsk(10);
	SVC_PERROR(wup_tsk(TASK1_3));
	SVC_PERROR(wup_tsk(TASK1_3));
	dly_tsk(20);

	/* 周期ハンドラ起動 */
	SVC_PERROR(sta_cyc(CYCHDR1));
	SVC_PERROR(sta_cyc(CYCHDR2));
	dly_tsk(100); 
	SVC_PERROR(stp_cyc(CYCHDR1));
	SVC_PERROR(stp_cyc(CYCHDR2));

	/* アラームハンドラ起動 */
	SVC_PERROR(sta_alm(ALMHDR1, 10));
	SVC_PERROR(sta_alm(ALMHDR2, 10));
	dly_tsk(20); /* 待つ */

	SVC_PERROR(sta_alm(ALMHDR1, 20));
	SVC_PERROR(sta_alm(ALMHDR2, 20));
	dly_tsk(40); /* 待つ */
...
//	SVC_PERROR(syslog_msk_log(LOG_UPTO(LOG_INFO), LOG_UPTO(LOG_EMERG)));
	 ...
	 */
#ifdef TASK_LOOP
	task_loop[0] = TASK_LOOP;
	task_loop[1] = TASK_LOOP;
#else /* TASK_LOOP */
