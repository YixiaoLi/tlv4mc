...
以下にJsonにおけるオブジェクトの例を示す．

\begin{File}{Jsonにおけるオブジェクトの例}{JsonObject}
{
  "Image":{
    "Width":  800,
    "Height": 600,
...
\begin{File}{Jsonにおける配列の例}{JsonArray}
[
  {
    "City":      "SAN FRANCISCO",
    "State":     "CA",
...
TOPPERS/ASPカーネルのトレースログの例を次に示す．

\begin{File}{変換元となるTOPPERS/ASPカーネルのトレースログの例}{aspSample}
...
\begin{File}{表\ref{aspSample}を標準形式トレースログで表現した例}{aspSample}
...
図の形状が線分({\tt Line})，矢印({\tt Arrow})のときは始点と終点，多角形({\tt Polygon})は各頂点を{\tt Points}というメンバを使い座標を指定する．
{\tt Points}の説明は次の通り．
...
注目するべきイベントの発生時刻にマーカーを配置することで，理解の補助することが出来る．
